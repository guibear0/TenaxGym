...
export default function BMInfo({ peso, altura }) {
  ...
  const getInterpretacion = (valor) => {
    ...
    if (valor >= 18.5 && valor < 25)
      return { texto: "Normal", color: "text-green-400" };
    if (valor >= 25 && valor < 30)
      return { texto: "Sobrepeso", color: "text-yellow-400" };
      ...
      >
        ...
        <AnimatePresence>
          {mostrarInf && (
            ...
            >
              <p className="mb-3">
                El IMC se calcula dividiendo el peso (kg) entre la altura al
                cuadrado (m²). Indica el estado nutricional general.
              </p>
              ...
              <i>
                Asegúrate de discutir preguntas específicas sobre tu salud y
                tratamientos con un profesional de la salud.
              </i>
            </motion.div>
          )}
        </AnimatePresence>
